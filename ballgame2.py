...
class Ball(pygame.sprite.Sprite):
    ...
    def __init__(self, filename, paddle, blocks, score, speed, angle_left, angle_right):
        ...
        self.paddle = padlle    #パドルへの参照
        self.blocks = blocks    #ブロックグループへの参照
        self.update = self.start #ゲーム開始状態に更新
        self.score = score
        self.hit = 0    #連続でブロックを壊した回数
        self.soeed = speed  #ボールの初期速度
        self.angle_left = angle_left    #パドルの反射方向（左端:135°）
        self.angle_right = angle_right  #パドルの反射方向（右端:45°）
    ...
    #パドルとの反射（左端:135°方向、右端:45°方向、それ以外:線形補間
    #２つのspriteが接触しているかどうかの判定
        if self.rect.colliderct(self.paddle.rect) and self.dy >0:
            self.hit = 0            #連続ヒットを０に戻す
...
class Block(pygame.sprite.Sprite):
    def __init__(self, filename, x, y):
        pygame.sprite.Sprite.__init__(self, self.containers)
        self.image = pygame.image.load(filename).covert()
        self.rect = self.image.get_rect()
        #ブロックの左上座標
        self.rect.left = SCREEN.left + x * self.rect.width
        self.rect.top = SCREEN.top + y * self.height
...
class Score():
    ...
    def draw(self, screen):
        img = self.sysfont.render("SCORE:"+ste(self.score), True, (255,255,250))
...
def main():
    ...
    #Ball.paddle_sound = pygame.mixer.Sound("C:/github/sample/python/pygame/breakout/flashing.wav")     #パドルにボールが衝突したときの効果音取得
    #Ball.block_sound = pygame.mixer.Sound("C:/github/sample/python/pygame/breakout/flying_pan.wav")       #ブロックにボールが衝突した時の効果音取得
    #Ball.gameover_sound = pygame.mixer.Sound("C:/github/sample/python/pygame/breakout/baded1.wav")       #ゲームオーバー時の効果音取得
    #　描画用のスプライトグループ
    ...
    #ボールを作成
    Ball("C:/github/sample/python/pygame/breakout/ball.png", paddle, blocks, score, 5, 135, 45)
